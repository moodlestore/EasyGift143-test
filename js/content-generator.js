...
window.ContentGenerator = {
    ...
    getHTML: function() {
        return `
            <!-- 서브탭 네비게이션 -->
            <div class="sub-tabs">
                <button class="sub-tab active" data-tab="gift-story" onclick="ContentGenerator.switchSubTab('gift-story')">
                    📖 감동 선물 사연
                </button>
                <button class="sub-tab" data-tab="new-feature" onclick="ContentGenerator.switchSubTab('new-feature')">
                    ➕ 새 기능
                </button>
            </div>

            <!-- 서브탭 컨텐츠 -->
            <div id="gift-story" class="sub-tab-content active">
                ${this.getGiftStoryHTML()}
            </div>

            <div id="new-feature" class="sub-tab-content" style="display: none;">
                ${this.getNewFeatureHTML()}
            </div>

            <!-- 웹훅 설정 모달 -->
            <div id="webhookModal" class="modal" style="display: none;">
                <div class="modal-content">
                    <span class="close" onclick="ContentGenerator.closeWebhookModal()">&times;</span>
                    <h2>⚙️ 웹훅 설정</h2>
                    
                    <div class="form-group">
                        <label>웹훅 1 - 컨텐츠 생성:</label>
                        <div class="url-input-group">
                            <input type="text" id="webhook1Url" placeholder="컨텐츠 생성 웹훅 URL">
                            <button onclick="ContentGenerator.saveWebhook(1)">저장</button>
                        </div>
                        <span id="webhook1Saved" class="saved-indicator" style="display: none;">✅ 저장됨</span>
                    </div>
                    
                    <div class="form-group">
                        <label>웹훅 2 - 이미지 재생성:</label>
                        <div class="url-input-group">
                            <input type="text" id="webhook2Url" placeholder="이미지 재생성 웹훅 URL">
                            <button onclick="ContentGenerator.saveWebhook(2)">저장</button>
                        </div>
                        <span id="webhook2Saved" class="saved-indicator" style="display: none;">✅ 저장됨</span>
                    </div>
                    
                    <div class="form-group">
                        <label>웹훅 3 - 컨텐츠 전송:</label>
                        <div class="url-input-group">
                            <input type="text" id="webhook3Url" placeholder="컨텐츠 전송 웹훅 URL">
                            <button onclick="ContentGenerator.saveWebhook(3)">저장</button>
                        </div>
                        <span id="webhook3Saved" class="saved-indicator" style="display: none;">✅ 저장됨</span>
                    </div>
                    
                    <div class="modal-actions">
                        <button onclick="ContentGenerator.closeWebhookModal()" class="btn-secondary">닫기</button>
                    </div>
                </div>
            </div>
        `;
    ...
    // 감동 선물 사연 HTML
    getGiftStoryHTML: function() {
        return `
            <div class="gift-story-container">
                <!-- 액션 버튼 영역 -->
                <div class="action-buttons" style="display: flex; gap: 15px; margin-bottom: 20px;">
                    <button id="generateBtn" onclick="ContentGenerator.generateContent()">🎬 컨텐츠 생성</button>
                    <button onclick="ContentGenerator.clearContent()">🔄 초기화</button>
                    <button id="submitBtn" onclick="ContentGenerator.submitContent()">📤 컨텐츠 전송</button>
                    <button onclick="ContentGenerator.openWebhookModal()">⚙️ 설정</button>
                </div>
                ...
                <div class="image-section">
                    ...
                    <div class="image-tabs" style="display: flex; margin-bottom: 15px;">
                        <button class="image-tab active" data-tab="photo" onclick="ContentGenerator.switchImageTab('photo')">📷 사진</button>
                        <button class="image-tab" data-tab="prompt" onclick="ContentGenerator.switchImageTab('prompt')">📝 프롬프트</button>
                    </div>
                    ...
                    <div id="prompt-tab" class="image-tab-content" style="display: none;">
                        ...
                        <div class="prompt-actions" style="margin-top: 15px; text-align: center;">
                            <button id="regenerateBtn" onclick="ContentGenerator.regenerateImage()">🖼️ 이미지 재생성</button>
                        </div>
                    </div>
                </div>
    ...
    // 새 기능 HTML (확장용)
    getNewFeatureHTML: function() {
        return `
            <div class="section">
                <h2>🚧 새로운 기능 개발 중</h2>
                <div style="text-align: center; padding: 60px 20px; color: #666;">
                    <h3>곧 새로운 기능이 추가될 예정입니다!</h3>
                    <p>다음과 같은 기능들을 준비하고 있어요:</p>
                    <ul style="text-align: left; display: inline-block; margin-top: 20px;">
                        <li>🎯 계절별 테마 컨텐츠 생성</li>
                        <li>💌 관계별 맞춤 메시지 생성</li>
                        <li>🎨 다양한 이미지 스타일 선택</li>
                        <li>📊 컨텐츠 성과 분석</li>
                        <li>🔄 일괄 컨텐츠 생성</li>
                    </ul>
                    <p style="margin-top: 30px;">업데이트를 기대해 주세요! 🙏</p>
                </div>
            </div>
        `;
    },
    ...
    loadSavedWebhooks: function() {
        this.webhookUrls.contentGeneration = Utils.safeStorage.get('contentGenerationWebhook', '');
        this.webhookUrls.imageRegeneration = Utils.safeStorage.get('imageRegenerationWebhook', '');
        this.webhookUrls.contentSubmission = Utils.safeStorage.get('contentSubmissionWebhook', '');

        // UI에 반영
        setTimeout(() => {
            const webhook1 = document.getElementById('webhook1Url');
            const webhook2 = document.getElementById('webhook2Url');
            const webhook3 = document.getElementById('webhook3Url');

            if (webhook1 && this.webhookUrls.contentGeneration) {
                webhook1.value = this.webhookUrls.contentGeneration;
                this.showSavedIndicator('webhook1Saved');
            }
            if (webhook2 && this.webhookUrls.imageRegeneration) {
                webhook2.value = this.webhookUrls.imageRegeneration;
                this.showSavedIndicator('webhook2Saved');
            }
            if (webhook3 && this.webhookUrls.contentSubmission) {
                webhook3.value = this.webhookUrls.contentSubmission;
                this.showSavedIndicator('webhook3Saved');
            }
        }, 100);
    },
    ...
    setupEventListeners: function() {
        ...
        console.log('ContentGenerator 이벤트 리스너 설정 완료');
    },
    ...
    saveWebhook: function(webhookNumber) {
        const urlInput = document.getElementById(`webhook${webhookNumber}Url`);
        const indicator = document.getElementById(`webhook${webhookNumber}Saved`);
        ...
        // 저장
        const webhookKeys = ['', 'contentGenerationWebhook', 'imageRegenerationWebhook', 'contentSubmissionWebhook'];
